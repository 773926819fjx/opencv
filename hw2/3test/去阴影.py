...
def median_filter(img: np.ndarray, point: np.ndarray,
                  filter_size: int) -> List:
    indices = [[x, y] for x in range(point[1] - filter_size // 2, point[1] +
                                     filter_size // 2 + 1)
               for y in range(point[0] - filter_size // 2, point[0] +
                              filter_size // 2 + 1)]

    indices = list(
        filter(
            lambda x: not (x[0] < 0 or x[1] < 0 or x[0] >= img.shape[0] or x[1]
                           >= img.shape[1]), indices))
    ...
    for channel in range(3):
        pixel_values[channel] = list(img[index[0], index[1], channel]
...
def edge_median_filter(img: np.ndarray,
    ...
    for partition in contours_list:
        for point in partition:
            temp_img[point[0][1]][point[0][0]] = median_filter(
                img, point[0], filter_size)

    return cv.cvtColor(temp_img, cv.COLOR_HSV2BGR)


def display_region(org_image: np.ndarray, shadow_clear_image: np.ndarray,
                   label: int, label_region: np.ndarray,
                   contours: tuple) -> None:
...
def correct_region_lab(org_img: np.ndarray, shadow_clear_img: np.ndarray,
    ...
    shadow_average_lab = np.mean(org_img[shadow_indices[0],
                                         shadow_indices[1], :],
                                 axis=0)

    # Get the average LAB from border areas
    border_average_lab = np.mean(org_img[non_shadow_indices[0],
                                         non_shadow_indices[1], :],
                                 axis=0)
...
def correct_region_bgr(org_img: np.ndarray, shadow_clear_img: np.ndarray,
    ...
    shadow_average_bgr = np.mean(org_img[shadow_indices[0],
                                         shadow_indices[1], :],
                                 axis=0)

    # Get the average BGR from border areas
    border_average_bgr = np.mean(org_img[non_shadow_indices[0],
                                         non_shadow_indices[1], :],
                                 axis=0)
...
def process_regions(org_image: np.ndarray, mask: np.ndarray,
                    lab_adjustment: bool, shadow_dilation_kernel_size: int,
                    shadow_dilation_iteration: int, shadow_size_threshold: int,
                    verbose: bool) -> np.ndarray:
    lab_img = cv.cvtColor(org_image, cv.COLOR_BGR2LAB)
    shadow_clear_img = np.copy(
        org_image)  # Used for constructing corrected image

    # We need connected components
    # Initialize the labels of the blobs in our binary image
    labels = measure.label(mask)

    non_shadow_kernel_size = (shadow_dilation_kernel_size,
                              shadow_dilation_kernel_size)
    non_shadow_kernel = cv.getStructuringElement(cv.MORPH_ELLIPSE,
                                                 non_shadow_kernel_size)
    ...
    for label in np.unique(labels):
        if not label == 0:
            ...
            if cv.countNonZero(temp_filter) >= shadow_size_threshold:
                ...
                shadow_clear_img = edge_median_filter(
                    cv.cvtColor(shadow_clear_img, cv.COLOR_BGR2HSV), contours)
                if verbose:
                    display_region(org_image, shadow_clear_img, label,
                                   temp_filter, contours)
...
def calculate_mask(org_image: np.ndarray,
    ...
    if sum(means[1:]) <= channel_max:
        mask = cv.inRange(lab_img, (0, 0, 0),
                          (thresholds[0], channel_max, channel_max))
    else:  # Else, also consider B channel
        mask = cv.inRange(lab_img, (0, 0, 0),
                          (thresholds[0], channel_max, thresholds[2]))
...
def remove_shadows(org_image: np.ndarray, lab_adjustment: bool,
    ...
    shadow_clear_img = process_regions(org_image, mask, lab_adjustment,
                                       shadow_dilation_kernel_size,
...
def process_image_file(
    ...
    if save:
        f_name = img_name[:img_name.index(".")] + "_shadowClear" + img_name[
            img_name.index("."):]
