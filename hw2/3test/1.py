...
def max_filter(image, filter_size):
    ...
    empty_image = np.full(
        (image.shape[0] + (filter_size // 2) * 2, image.shape[1] +
         (filter_size // 2) * 2), -1)
    # 将原图像填充进矩阵
    empty_image[(filter_size // 2):empty_image.shape[0] - (filter_size // 2),
                (filter_size //
                 2):empty_image.shape[1] - (filter_size // 2)] = image.copy()
    # 创建结果矩阵，和原图像大小相同
    ...
    for h in range(filter_size // 2, empty_image.shape[0] - filter_size // 2):
        for w in range(filter_size // 2,
                       empty_image.shape[1] - filter_size // 2):
            filter = empty_image[h - (filter_size // 2):h +
                                 (filter_size // 2) + 1, w -
                                 (filter_size // 2):w + (filter_size // 2) + 1]
            result[h - filter_size // 2,
                   w - filter_size // 2] = np.amax(filter)
    return result
    ...
    for h in range(filter_size // 2, empty_image.shape[0] - filter_size // 2):
        for w in range(filter_size // 2,
            ...
            filter = empty_image[h - (filter_size // 2):h +
                                 ...
                                 (filter_size // 2):w + (filter_size // 2) + 1]
            result[h - filter_size // 2,
                   w - filter_size // 2] = np.amin(filter)
    return result
...
if __name__ == "__main__":
    result = remove_shadow('hw2\\3\\28.jpg')
    cv2.imwrite("hw2\\3test\\out28.jpg", result)
